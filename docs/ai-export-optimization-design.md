...
11. **export_events への章粒度（フラグメント構成）の追加**: 現行確定仕様（[export-events-design.md](export-events-design.md)）は資料項目単位のスナップショットのみを記録し、Phase 3D でも変更しない。章分割・フラグメント構成は manifest.md に残る。AI 成果物の帰還で章粒度の紐づけが必要になるかは Phase 4 の設計時に再検討する
...
### Phase 3Eへ引き継いだ残課題（Should）

- **Gemini向けの案内**: 書き出しモーダルの説明文はChatGPT・Claude・NotebookLMのみ例示し、Geminiの案内がない
- **NotebookLM利用手順**: NotebookLMはZIPを直接受け付けないため、解凍してから個別アップロードする案内が必要
- **トークン概算表示の統一**: 資料棚ヘッダー/資料一覧の丸め表記（約4.5万トークン）と、書き出しプレビューモーダルの精緻な表記（推定トークン数: 45,231）で精度感が異なる
- **AI書き出しのPlaywright E2E**: 書き出しモーダル（プロファイル選択・プレビュー・ダウンロード）のブラウザレベル回帰テストが0件
...
Phase 3A〜3DのMust課題（階層TOCの章分割重複）は解消済みであり、Phase 3A〜3Dの完了状態はそのまま維持する。NotebookLM対応はmasterへ統合可能な品質に達している。Phase 3EはUXの磨き込み（上記Should項目とトークン係数・E2Eの実測調整）と位置づけ、Phase 4以降の設計へ進める状態にある。
