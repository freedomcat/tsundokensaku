...
- **`render_chunk` は I/O を `RenderContext` の関数経由でのみ行う**: standard は注入された `render_pdf` / `render_markdown`（web.py の既存関数）をそのまま呼ぶ。chat/notebooklm は PDF 結合（pypdf `PdfWriter` へ複数項目のページを順に追加）・MD 連結（既存 `render_markdown_pages` を項目ごとに呼んで連結）を行う想定（未実装）。テストではフェイクの `resolve_pdf` / `render_pdf` / `render_markdown` を注入できる
