...
- 1 ファイルのページ数が上限（`NOTEBOOKLM_MAX_PAGES_PER_FILE`、初期値 300。§20 参照）を超える場合は項目単位で分割する
- 項目の途中（ページ範囲の途中）では絶対に分割しない
- 出力ファイル数が `NOTEBOOKLM_MAX_SOURCES`（初期値 50）を超える場合は警告する（出力自体は行う）
- 出典が分かるエントリ名 `{NN}_{書名}_p{範囲}.pdf` を維持し、複数ファイルは ZIP にまとめる
...
- 各ファイル冒頭に資料名・分冊番号（n/全m）・収録項目一覧（書名・元 PDF ファイル名・ページ範囲・推定トークン数）を記載する。本文は既存 `render_markdown_pages` と同じ「## 書名 → ### p.N」構造で項目ごとに連結する
...
- `profile=notebooklm` は format 不要（PDF 固定）、`profile=chat` も不要（MD 固定）。`profile` と矛盾する `format` が同時指定された場合は 400（誤用を黙って解釈しない）
...
- エラー: profile 指定時も空資料 400 / ページ未指定 400 / PDF 欠損 404 は standard と同じ（実行 API は従来どおり厳格、プレビュー API だけ寛容）
...
```python
# RenderContext（web.py が組み立てて渡す。プロファイルに DB 接続を持たせない）
@dataclass(frozen=True)
class RenderContext:
    pack_name: str
    exported_at: datetime
    resolve_pdf: Callable[[str], Path]          # _resolve_pdf_file_or_404 相当
    load_texts: Callable[[Path, list[int]], dict[int, str]]  # load_pages_text 相当
```
...
- **`render_chunk` は I/O を `RenderContext` の関数経由でのみ行う**: PDF 結合は pypdf `PdfWriter` へ複数項目のページを順に追加、MD 連結は既存 `render_markdown_pages` を項目ごとに呼んで連結。テストではフェイクの `resolve_pdf` / `load_texts` を注入できる
...
| B-2 | `api_export_pack` の項目ループを `StandardProfile` 経由へ載せ替え（`RenderContext` 導入） | `web.py` | 既存テストで回帰確認（無修正で通す） | B-1 |
...
| # | ステップ | 変更対象 | テスト | 依存 |
|---|---|---|---|---|
| C-1 | `ChatProfile`（トークン重み・上限・分割・警告・命名） | `export_profiles.py` | `test_export_profiles.py` 追記 | B-1 |
| C-2 | MD 連結レンダラ（出典ヘッダ + 項目連結）と `render_chunk` | `markdown_export.py`, `export_profiles.py` | `test_markdown_export.py` 追記 | C-1 |
| C-3 | プレビュー API へ profile・分割予定・警告を反映 | `web.py` | `test_web.py` 追記 | A-3, C-1 |
| C-4 | export API の `profile=chat` 出力（拡張 manifest 含む） | `web.py`, `zip_export.py` | `test_web.py` / `test_zip_export.py` 追記 | B-3, C-2 |
| C-5 | UI: エクスポートモーダル（宛先選択・概算・分割予定・警告。3A の常時表示は要約に縮小） | `workspace.html` | 手動確認 | C-3, C-4 |

**完了条件**: モーダルから chat エクスポートが完了し、各 MD に資料名・書名・元 PDF・ページ範囲が入っている。1 項目超過の警告と単独出力が動く。
...
| # | ステップ | 変更対象 | テスト | 依存 |
|---|---|---|---|---|
| D-1 | `NotebookLMProfile`（ページ重み・上限・隣接同一書籍の結合判断・ソース数警告・命名） | `export_profiles.py` | `test_export_profiles.py` 追記 | B-1 |
| D-2 | PDF 結合レンダラ（複数項目 → 1 PDF、pypdf） | `pdf_export.py`, `export_profiles.py` | `test_export_pdf_pages.py` 追記 | D-1 |
| D-3 | export API / プレビュー / モーダルへの notebooklm 追加 | `web.py`, `workspace.html` | `test_web.py` 追記 | C-3〜C-5, D-2 |

**完了条件**: notebooklm エクスポートが完了し、隣接する同一書籍の項目が 1 PDF に結合され、manifest とファイル名から出典が追える。50 ファイル超で警告が出る。
...
2. **notebooklm のページ上限既定値**: NotebookLM の公称制限は「1 ソース 50 万語 / 200MB」でページ数基準ではない。初期値 300 ページは仮置きであり、実際のアップロード検証で調整する
...
6. **巨大資料（メモリ上 ZIP 構築の限界）**: 現行から続く制約。GB 級の資料が現実に発生するかを見てから streaming 化を検討する
