...
- Playwright全件成功。
- 本設計書の記述と実装が一致している。
- R7-1実装完了時点のROADMAPでは、R7-1を完了、R7-2〜R7-4を未設計・未実装として要約していた。現在はR7-1が詳細設計済み・実装済み、R7-2〜R7-4が詳細設計済み・未実装であり、R7親項目・`web.py`の責務分離親項目は未完了のまま維持する（R7-2〜R7-4・R5・R8が残るため）。

### 残るR7子責務

現在、R7-2（PDFディレクトリ取り込み）は[R7-2専用文書](r7-2-pdf-directory-import.md)、R7-3（Scrapbox JSON保存・同期）は[R7-3専用文書](r7-3-scrapbox-sync.md)、R7-4（PDF閲覧・変換・本文検索のHTTPオーケストレーション）は[R7-4専用文書](r7-4-http-orchestration.md)で詳細設計済み・未実装である。R7親項目は未完了のままとする。R7-2 → R7-3 → R7-4の順に行った個別設計前の調査記録は、§3「R7-2〜R7-4横断調査」を参照する。
