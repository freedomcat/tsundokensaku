...
R7の全体像、共通する責務境界・依存方向・進捗は§3を正とし、実装済みまたは詳細設計済みの子責務は次の専用文書を正本とする。

#### R7-1: PDFアップロード保存 — 完了

- 詳細設計と実装結果: [R7-1 PDFアップロード保存](refactoring/r7-1-pdf-upload-storage.md)
- `pdf_import_service.save_uploaded_pdf(filename, content, books_dir, *, relative_path=None) -> Path`へファイル配置責務を分離済み。
- `web.py`にはHTTP入力、demo mode判定、bodyと`%PDF`の検証、例外のHTTP 400変換、HTTP 201レスポンスを維持する。

#### R7-2: PDFディレクトリ取り込み — 詳細設計済み・未実装

- 詳細設計: [R7-2 PDFディレクトリ取り込み](refactoring/r7-2-pdf-directory-import.md)
- 分離先はR7-1と同じ`pdf_import_service.py`、公開APIは`import_pdfs_from_directory(source, books_dir) -> PdfDirectoryImportResult`として確定済み。
- 設計のみが完了しており、characterization test追加と責務分離の実装は未着手。
...
- R7（`pdf_service`）→ 副作用が大きいので最後。**この記述は策定当時の一括り判断の歴史的記録。R7はその後子責務分解され、R7-1（PDFアップロード保存）は完了、R7-2〜R7-4は詳細設計済み・未実装となった。詳細は§3「R7の子責務分解」と[R7-1](refactoring/r7-1-pdf-upload-storage.md)・[R7-2](refactoring/r7-2-pdf-directory-import.md)・[R7-3](refactoring/r7-3-scrapbox-sync.md)・[R7-4](refactoring/r7-4-http-orchestration.md)の各専用文書を参照。**
