...
**つんどけんさくは、人やAIが蔵書を活用しやすくするためのローカル知識アクセス基盤です。**

積読になっているPDFの中から、問いに関係する箇所をPDF本文の全文検索で見つけ、必要なページを選んで資料として組み立て、PDFやMarkdownとして書き出せます。書き出した資料は、ChatGPTやClaude、NotebookLMなどの外部AIサービスや、Scrapbox・Obsidianといった外部ノートサービスなど、利用者が普段使っているツールへそのまま渡せます。

想定しているのは、自分が利用権限または利用許諾を持つ多数のPDF資料から、問いに関係する箇所を探して活用したい個人利用者です。

PDFやその他のデータが外部サービスへ自動送信されることはありません。検索・OCR・資料の組み立てはすべてローカル環境で完結し、何を・どこへ渡すかは利用者自身が選択・確認したうえで書き出します。中心的な価値は、眠っている積読を検索可能にし、必要な箇所をすぐに取り出せる状態へ変えることです。

## 背景

このツールは [masui-shelf](https://scrapbox.io/masui-shelf/) に触発されて作りました。  
Scrapboxでの蔵書管理は書誌情報の一覧性に優れていますが、PDFの本文内容までは検索できません。  
つんどけんさくは、その欠けている部分である PDF 本文の全文検索から始まり、見つけた箇所を問いごとの資料として組み立て、外部のAIやノートサービスなど、利用者が使うツールで活用できる形にするところまでを支えるツールです。
...
つんどけんさくは、Web UI・CLI・HTTP APIを通じた操作以外で外部サービスと通信しません。`PDF_EXPORT_SAVE_DIR` にGoogle Drive等の同期フォルダを指定した場合も、つんどけんさく自身はローカルのフォルダへファイルを書き出すだけです。そのフォルダをどこかへ同期するかどうかは、利用者が導入した同期ソフト側の責務であり、つんどけんさくの機能ではありません。
...
Web UIが内部で使っているものと同じHTTP APIを直接呼び出すこともできます。HTTP APIは、蔵書の検索・資料机での資料操作・出典付きエクスポートなど、Web UIと同じ範囲の操作をスクリプトや他のツールから利用するための手段です。CLI（`python -m tsundokensaku`）は現時点でインデックス作成・検索・タイトル再推定のみに対応しており、資料作成やエクスポートへの対応は今後の課題です。
...
現時点では、ローカルで安定して動く全文検索と、人やAIなど外部ツールへ渡すページを自分の蔵書から探して切り出す体験を優先しています。

## 利用上の注意

つんどけんさくは、利用者が利用権限または利用許諾を持つ資料を、自分の知識活用のために検索・整理することを目的としたツールです。

- 著作権法および各サービスの利用規約の遵守は利用者の責任です。
- アクセス制限の回避など、不正な手段でコンテンツを取得する機能は提供しません。
- 利用権限または利用許諾を持たない資料の利用は想定していません。
- 外部AIサービスや外部ノートサービスへ資料を渡す場合も、利用する資料や各サービスの利用条件を確認してください。
