...
- `packs`, `pack_items`: ワークスペースの名前付きパック（本ごとのページ範囲構成）
- `app_state`: アプリ状態のキー値保存（現在はアクティブパックIDのみ）
...
- 検索結果のPDFヒットの「資料に追加」で本単位に追加します。検索ページのバーは件数表示とワークスペースへの導線だけで、エクスポートはワークスペースから行います
- データは「パック」（UI上の表記は「資料」。組み立てた資料そのものを指すため）としてSQLite（`packs` / `pack_items` / `app_state` テーブル）に永続化します。クライアントは `/api/packs` 系APIでアクティブパックを読み書きし、メモリキャッシュ上では旧sessionStorage時代と同じ `{ version: 2, books: { "<pdf_path>": { title, pages, collapsed, addedAt } } }` 形式を保ちます。`pages` はページ指定spec文字列（例: `3-7,20-35`）そのままで、開区間（`39-`）も表現できます
- パックは名前を付けて複数持て、ワークスペースのセレクトボックスで切替・新規作成・改名・削除ができます。検索結果やPDFプレビューからの追加はアクティブパックへ入ります。旧sessionStorageカートは初回アクセス時に「移行されたワークスペース」パックとして自動で取り込みます。詳細設計は [docs/pack-design.md](docs/pack-design.md)
...
- 各本カードの「PDFで選ぶ」から既存のPDFプレビューモーダルを開き、本文を目視確認しながらページ欄で範囲を指定して「資料に追加」できます。このボタンはワークスペース起点で開いたときだけ表示します
- 「ページを追加」モーダルで、ワークスペース内のPDF本文をキーワード検索してページを追加できます。各本カードのボタンからはその本だけを対象に開き、モーダル内のセレクタで「すべての本」と個別の本を切り替えられます。`GET /search-pages?pdf_path=...&q=...` が1冊分の `pages` テーブルを部分一致検索し、ページ番号と抜粋を返します。PDFアウトラインの有無に依存しないため、BookScan由来のアウトラインなしPDFでも使えます。追加済みページは選択不可で表示し、重複追加を防ぎます
- エクスポートは本ごとに1ファイルで、既存の `/export-pdf` と `/export-md` を順に呼び出すだけです。パックの読み書きは `/api/packs` 系APIが担い、`GET /workspace` 自体はテンプレートを返すだけです
- 複数冊では連続ダウンロードになるため、ブラウザが複数ファイルのダウンロード許可を求めることがあります

ページ指定specの解析・結合・検証・圧縮は `static/pages-spec.js` に共通化し、PDFモーダル・検索ページ・ワークスペースで共用しています。パックの読み書き（サーバ同期・デバウンス書込み・旧カート移行）とナビのバッジ更新は `static/pack-store.js` が担当します。
