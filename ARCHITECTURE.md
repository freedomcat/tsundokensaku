...
- 検索結果のPDFヒットの「ワークスペースに追加」で本単位に追加します。検索ページのバーは件数表示とワークスペースへの導線だけで、エクスポートはワークスペースから行います
- データは「パック」としてSQLite（`packs` / `pack_items` / `app_state` テーブル）に永続化します。クライアントは `/api/packs` 系APIでアクティブパックを読み書きし、メモリキャッシュ上では旧sessionStorage時代と同じ `{ version: 2, books: { "<pdf_path>": { title, pages, collapsed, addedAt } } }` 形式を保ちます。`pages` はページ指定spec文字列（例: `3-7,20-35`）そのままで、開区間（`39-`）も表現できます
...
- 各本カードの「PDFで選ぶ」から既存のPDFプレビューモーダルを開き、本文を目視確認しながらページ欄で範囲を指定して「ワークスペースに追加」できます。このボタンはワークスペース起点で開いたときだけ表示します
