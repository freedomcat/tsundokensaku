...
- 検索結果からのエクスポートカート（本ごとにPDF/Markdown一括出力）
- インデックス実行と進捗表示
...
検索結果のPDFヒットに「カートに追加」チェックボックスを付け、複数の本・ページをまとめて選んでからPDFまたはMarkdownを一括ダウンロードできます。

- カートは `sessionStorage` に本単位（`pdf_path` → タイトルとページ番号集合）で保持し、検索し直しやリロード後もタブ内では維持されます
- エクスポートは本ごとに1ファイルで、既存の `/export-pdf` と `/export-md` を順に呼び出すだけです。サーバ側にカート専用の処理はありません
- ページ番号の集合は `2,3,4,9` を `2-4,9` に圧縮してから既存のページ指定形式で渡します
- 複数冊を選ぶと連続ダウンロードになるため、ブラウザが複数ファイルのダウンロード許可を求めることがあります
